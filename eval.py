...
class SegmentationMetric(object):
    ...
    def pixelAccuracy(self):
        ...
        self.confusionMatrix = self.confusionMatrix[0:5, 0:5] # if V dataset error, modify this code
...
def evaluate1(pre_path, label_path):
    ...
    for i, p in enumerate(pre_imgs):
        ...
        metric = SegmentationMetric(6)  # Represents the number of categories, including background
...
if __name__ == '__main__':
    pre_path = './results/{}/GAGNet/'.format(DATASET)
    ...
    if DATASET == "Vaihingen":
        print("Imp.surf:{:.2f}%\nBuildings:{:.2f}%\nLow veg.:{:.2f}%\nTree:{:.2f}%\nCar:{:.2f}%".format(np.mean(classAcc[0])*100,np.mean(classAcc[1])*100,np.mean(classAcc[2])*100,np.mean(classAcc[3])*100,np.mean(classAcc[4])*100))
        print("Imp.surf IoU:{:.2f}%\nBuildings IoU:{:.2f}%\nLow veg. IoU:{:.2f}%\nTree IoU:{:.2f}%\nCar IoU:{:.2f}%".format(np.mean(IoU_list[0])*100,np.mean(IoU_list[1])*100,np.mean(IoU_list[2])*100,np.mean(IoU_list[3])*100,np.mean(IoU_list[4])*100))
    else:
        print(
            "Imp.surf:{:.2f}%\nBuildings:{:.2f}%\nLow veg.:{:.2f}%\nTree:{:.2f}%\nCar:{:.2f}%\nClutter:{:.2f}%".format(
                np.mean(classAcc[0]) * 100, np.mean(classAcc[1]) * 100, np.mean(classAcc[2]) * 100,
                np.mean(classAcc[3]) * 100, np.mean(classAcc[4]) * 100, np.mean(classAcc[5]) * 100))
        print(
