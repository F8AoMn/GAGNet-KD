...
from GAGNet-KD.GAGNet import FFNet
...
if __name__ == "__main__":
    ...
    net.load_state_dict(torch.load('./weight/GAGNet-{}-loss.pth'.format(DATASET)))

    _, all_preds, all_gts = test(net, test_ids, all=True, stride=32)
    # print(accuary)
    for p, id_ in zip(all_preds, test_ids):
        img = convert_to_color_(p)
        io.imsave('./results/Vaihingen/GAGNet/top_mosaic_09cm_area{}.png'.format(id_), img)
        # io.imsave('./results/Potsdam/GAGNet/top_mosaic_09cm_area{}.png'.format(id_), img)
